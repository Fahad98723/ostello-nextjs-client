    ...
    if (!isEmpty(input1) && !isEmpty(input2) && !isEmpty(input3)) {
      ...
      try {
        if (video?.name?.length > 0) {
          let videosUp = await SocketFileUploader([video]);
          let thumbnailsUp = await SocketFileUploader([thumbnail]);
        ...
        if (image?.name?.length > 0) {
          images = await SocketFileUploader([image]);
        }

        let data = {
          name: input1,
          description: input3,
          instituteid: window.localStorage.getItem("INSTITUTE_ID"),
