...
import { isJsonParsable, SocketFileUploader } from "../../../../../utils/utils";
...
const MyProfile = () => {
      ...
      if (instituteDetails?.id) {
        try {
          const res = await axios.get(
            `${host}/institute/faculty?instituteId=${instituteDetails?.id}`
          );
  ...
  const saveServicesToState = (services) => {
    if (services["Junior Secondary School (Class 6-10th)"]) {
      setInstituteDomain((prv) => prv.concat("Junior Secondary School (Class 6-10th)"));
      setBoardsJrSchool(services["Junior Secondary School (Class 6-10th)"].boards);
      setClassesJrSchool(services["Junior Secondary School (Class 6-10th)"].classes);
      setSubjectsJrSchool(services["Junior Secondary School (Class 6-10th)"].subjects);
    }
    ...
    if (services["Senior Secondary School (Class 11-12th)"]) {
      setInstituteDomain((prv) => prv.concat("Senior Secondary School (Class 11-12th)"));
      setBoardsSrSchool(services["Senior Secondary School (Class 11-12th)"].boards);
      setStreamsSrSchool(services["Senior Secondary School (Class 11-12th)"].streams);
      setSubjectsSrSchool(services["Senior Secondary School (Class 11-12th)"].subjects);
  ...
  const removeHandleFaculty = (id) => {
    ...
    axios
      ...
      .catch((e) => {
        toast.error("Got an error! couldn't remove the faculty !");
        console.error(e);
      })
      .finally(() => {});
  };
  ...
  const handleInstituteUploadImages = async () => {
    ...
    try {
      socket
        .off("received_percentage")
        .on("received_percentage", (percentage) => {
      ...
      let images = await SocketFileUploader(instituteImage);
      console.log(images);
            ...
            {isDisable ? (
              ...
              <div className="flex space-x-2 ml-auto">
                ...
                <button
                  ...
                  onClick={() => {
                    // if (
                    //   // !isEmpty(instituteNameError)
                    //   // !isEmpty(contactNoError)
                    //   // !isEmpty(descriptionError)
                    //   // !isEmpty(instituteEmailError)
                    //   // !isEmpty(typeOfInstituteError) ||
                    //   // isEmpty(mode)
                    // ) {
                    //   console.log(
                    //     isEmpty(instituteNameError),
                    ...
                    console.log(countWords(instituteShortDescription));
