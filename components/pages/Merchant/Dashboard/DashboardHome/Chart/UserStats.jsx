...
export default function UserStats() {
  ...
  const fromDate = new Date(startValue)?.toISOString();
    ...
    useEffect(() => {
      const getDateData = async () => {
        const response = await axios.get(`${host}/analytics?instituteid=${INSTITUTE_ID}&fromdate=${fromDate}&todate=${toDate}`,{
          headers: {
  ...
  const handleByType = () => {
    let searchType = ''
    type === 1 ? searchType = 'visit_institute' :
                    type === 5 ? searchType = 'watch_videos' :
                    type === 4 ? searchType = 'research_faculties' :
                    type ===  3 ? searchType = 'explore_courses' :
                    type === 2 ? searchType = 'know_institute' : type === 6 ? searchType = 'visit_time' :  null
    const sortedType = merchantAnalytics
      ?.filter((obj) => obj?.activity_type?.includes(searchType))
    setData(sortedType)
    handleClose()
  }
  ...
  return (
    <div className='bg-white w-full mr-3 p-3 rounded-lg'>
        <div className='flex gap-x-1 md:px-5 px-3  justify-between'>
          <h3 className='font-bold text-sm text-[#252733]'>
            All User Stats
          </h3>
      ...
      <TableContainer sx={{ maxHeight: 440 }}>
        <table stickyHeader aria-label="sticky table" className='mt-10 table-auto'>
          ...
          <TableBody className='block max-h-[70vh]'>
            {data?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter(filterByTitle)?.map((d, index) => (
              <TableRow
                key={index}
              ...
              >
                <TableCell
                  className='px-6 py-4  font-sm text-[#252733]'
                >
                  <div className='flex items-center space-x-3'>
                    <p className='text-xs'>{d?.payload?.user_name || 'Anonymous'}</p>
                  </div>
                </TableCell>
                <TableCell
                  className='text-[#252733] font-sm px-6 py-4 whitespace-nowrap'
                >
                ...
                >
                  <div className='flex flex-col'>
                    <p className='font-bold text-xs'> {d?.areas?.length > 1 ? d?.areas[2] : 'N/A'}</p>
                  </div>
                </TableCell>
                ...
                <TableCell
                  className='text-[#252733] font-sm px-6 py-4 whitespace-nowrap flex items-center'
                >
                  {d?.activity_type === 'visit_institute' ? (
                    <div className='mr-2'>
                      <p className='text-xs'>{d?.payload?.institute_name}</p>
                    </div>
      ...
      />
                    ...
                    <div className='mr-2'>
                      <p className='text-xs'>Total Time: {d?.payload?.total_time_in_seconds} s</p>
                    </div>
                  ): (
                    <div className='r-2'>
                      <p className='text-xs'>{d?.payload?.institute_name}</p>
