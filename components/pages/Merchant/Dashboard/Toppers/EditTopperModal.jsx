...
import { isEmpty, SocketFileUploader } from "../../../../../utils/utils";
...
const EditTopperModal = ({ setOpen, open, singleTopper, instituteDetails }) => {
  ...
  const handleTopper = async () => {
    ...
    // let videosUploaded = await SocketFileUploader(videos);
    // let thumbnailsUploaded = await SocketFileUploader(thumbnails);
    // let imageUpload = await SocketFileUploader(images);

    // const videoFile = videosUploaded.map((item, index) => {
    //   return {
    ...
    const image = images[0]?.url ? images : await SocketFileUploader(images);
    const video = videos[0]?.url ? videos : await SocketFileUploader(videos);
    const thumbnail = thumbnails[0]?.url
      ? thumbnails
      : await SocketFileUploader(thumbnails);
