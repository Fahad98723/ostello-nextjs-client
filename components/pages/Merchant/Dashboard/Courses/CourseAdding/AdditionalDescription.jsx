...
const AdditionalDescription = ({
      ...
      if (instituteDetails?.id || courseData?.institute?.id) {
        try {
          const res = await axios.get(
            `${host}/institute/faculty?instituteId=${instituteDetails?.id || courseData?.institute?.id}`
          );
  ...
  useEffect(() => {
    if (Object.entries(additionalDescription).length) {
      ...
      setFaculty(
        facultyValues.find((a) => a.id === additionalDescription?.faculty)
      );
    }
    ...
    if (courseData && !Object.entries(additionalDescription).length) {
      ...
      if (!admin) {
        ...
        if (courseData?.updatedRequest?.faculties?.length) {
          let faculty = facultyValues?.find(
            (f) => f?.id === courseData.updatedRequest.faculties
          );
          if (faculty?.id?.length) {
            setFaculty(faculty);
          }
        }
      }
