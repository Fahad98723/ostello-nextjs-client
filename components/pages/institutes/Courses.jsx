...
const Courses = ({
  ...
  console.log(recentCourse, currentInstituteCourses);

  useEffect(() => {
    const sub = currentInstituteCourses
      ?.slice()
      .sort((a, b) => b?.videos?.length - a?.videos?.length)
      .filter((e) => e.category.classes.includes(selected));
    const sub2 = currentInstituteCourses
      ?.slice()
      .sort((a, b) => b?.videos?.length - a?.videos?.length)
  ...
  }, [currentInstituteCourses, selected]);

  useEffect(() => {
    const sub = currentInstituteCourses?.filter((e) =>
      e.category.classes.includes(selected)
    );
    const sub2 = currentInstituteCourses?.filter((e) =>
  ...
  useEffect(() => {
    if (recentCourse?.pricingdetails?.yearly) {
      setPriceRef("yearly");
    } else if (recentCourse?.pricingdetails?.halfYearly) {
      setPriceRef("halfYearly");
    } else if (recentCourse?.pricingdetails?.monthly) {
      setPriceRef("monthly");
    }
  }, [
    recentCourse?.pricingdetails?.yearly,
            ...
            <div className="md:flex md:max-w-[1350px] mx-auto px-5">
              <div className="md:w-2/4 md:flex pb-2">
                ...
                <div className="relative  my-1 md:w-[48%] md:ml-2 w-full">
                  ...
                  >
                    <p className={`  text-[16px] `}>
                      {courseSelected ? courseSelected : "Subjects"}
                    </p>
                    {subjectShowing ? (
                      <GoChevronDown className="ml-1 text-[16px] rotate-180" />
                    ) : (
                          ...
                          {course?.map((element, idx) => {
                            return (
                              ...
                              >
                                ...
                                {
                                  element?.videos?.length ? <AiFillVideoCamera className="text-[20px]"/> : ''
                                }
                              </div>
                            );
                          })}
