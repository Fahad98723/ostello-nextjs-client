...
const { metaSection } = router.query;
  ...
  const filterByArea = async (areaName, sortedInstitutes) =>{
   ...
   const filterByLocation = async (locName) => {
     try {
       const { data } = await axios.get(`${host}/institute?location=${locName === 'Jangpura' ? 'Jungpura' : "Lajpat Nagar" ? "Lagpat Nagar" : locName}&nolimit=true`)
  ...
  const filterBySearch = async (text,area) => {
   ...
   try {
     const { data } = await axios.get(`${host}/institute/query?name=${text}`)
