...
export default function ReviewCard({ data , type}) {
  ...
  return (
    <div className=' justify-center grid grid-cols-10 gap-0  items-center   transition-all duration-300 '>
      ...
      <div className={`hidden md:flex  md:col-span-6 col-span-10 items-center ${type === 'merchant-landing' ? 'h-[500px]' : 'h-[400px]'} h-[500px] bg-primary lg:w-full md:px-16 px-5 py-20 text-white rounded-tl-xl rounded-bl-xl`} >
        <div className=''>
          <div>
            <Rating readOnly value={rating} />
            <p className='text-[18px]'>{quote}</p>
      ...
      <div className='  md:col-span-4 col-span-10 w-full relative '>
        ...
        />  : <>
        <img
        src={type === 'merchant-landing' ? merchantReview.src : review.src}
        className={`w-full  m-0 ${type === 'merchant-landing' ? 'h-[500px]' : 'h-[400px]'} rounded-br-xl rounded-bl-xl md:hidden block`}
        alt=''
      />     
          <img
            src={type === 'merchant-landing' ? merchantReview.src : review.src}
            className={`w-full  m-0 h-[500px] ${type === 'merchant-landing' ? 'h-[500px]' : 'h-[400px]'} rounded-br-xl rounded-tr-xl md:block hidden `}
            alt=''
          />
          <img
            src={playIcon.src}
