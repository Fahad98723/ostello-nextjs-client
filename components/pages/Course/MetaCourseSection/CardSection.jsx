...
const CardSection = ({ metaSection, meta_area }) => {
  ...
  const [locationBy, setLocationBy] = useState(meta_area);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if(sortBy === 'Courses'){
        if(metaSection !== 'Science' && metaSection !== 'Academics' &&  metaSection !== 'Medical' && metaSection !== 'Engineering' && metaSection !== 'Law' && metaSection !== 'Skill Based' && metaSection !== 'Commerce'  && metaSection !== 'Humanities'){
  ...
  const filterByCategory = async (cat, area) => {
    try {
      ...
      const { data } = await axios.get(
        `${host}/institute?name=${searchText}&services=${query}&location=${area}&limit=50`
      );
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(
          `${host}/institute?name=${searchText}&services=${query}&limit=20`
        );
        const sortInstitutes = data?.message?.slice();
        // .sort((a, b) => b?.images?.length - a?.images?.length)
        // .sort((a, b) => b?.reviews?.length - a?.reviews?.length)
    ...
    run();
  }, [searchText, locationBy, metaSection]);
  ...
  return (
    <div className="px-5 sm:px-10">
      <div className="md:flex justify-between items-center my-3">
        <p className=" text-[48px] text-[#1D2939]">{metaSection}</p>
