      ...
      <Affix offsetBottom={0} className=" ">
        <div className="bg-white  flex justify-around py-1  rounded-xl rounded-b-none w-full ">
          ...
          {isAuthenticated ? (
            <div
              onClick={() => {
                router.push(`${userData.usertype === 3 ? '/profile' : userData.usertype === 1 ? '/admin-dashboard/overview' : 'merchant/dashboard/profile'}`);
                setActiveUrl(`${userData.usertype === 3 ? '/profile' : userData.usertype === 1 ? '/admin-dashboard/overview' : 'merchant/dashboard/profile'}`);
                dispatch(setShowSideBar(true));
                dispatch(setLogoForBottomMenu(false));
              }}
              className={` flex items-center justify-center flex-col mx-2 font-medium cursor-pointer  ${
                activeUrl === `${userData.usertype === 3 ? '/profile' : userData.usertype === 1 ? '/admin-dashboard' : 'merchant/dashboard/profile'}` ? "text-[#7D23E0]" : "text-gray-400"
              }`}
            >
