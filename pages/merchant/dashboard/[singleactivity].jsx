...
export default function SingleActivity() {
  ...
  useEffect(() => {
    const getSingleData = async () => {
      const response = await axios.get(`${host}/analytics?instituteid=${INSTITUTE_ID}&id=${router.query?.singleactivity
      }`,{
        headers: {
  ...
  }, [data?.payload?.userid,data?.payload?.courseid,data?.payload?.instituteid,data?.payload?.facultyid])

  const getUserName = async (userId) => {
    const response = await axios.get(`${host}/users/id?id=${userId}`,{
      headers: {
        'Access-Control-Allow-Origin': '*',
        Authorization: `Bearer ${
          typeof window !== 'undefined' &&
  ...
  const getFacultyName = async (facultyId) => {
    const response = await axios.get(`${host}/institute/faculty?id=${facultyId}`,{
      headers: {
        'Access-Control-Allow-Origin': '*',
        Authorization: `Bearer ${
          typeof window !== 'undefined' &&
  ...
  return (
    ...
    <Head>
      ...
      <div className='dashboard'>
        ...
        <p className="font-bold text-xl ml-5">User: {userName || 'Anonymous'}</p>
        <p className="font-bold text-xl ml-5">Location: {data?.areas?.length > 1 ? data?.areas[2] : 'N/A'}</p>
        <div className="ml-5">
        {data?.activity_type === 'visit_institute' ? (
                    <div className='mr-2'>
                      <p className='text-xl font-bold'>Institute: {instituteName}</p>
                    </div>
      ...
      />
                    ...
                    <div className='ml-5'>
                      <p className='text-xl font-bold'>Total Time: {data?.payload?.total_time_in_seconds} s</p>
                    </div>
                  ): (
                    <div className='ml-5'>
                      <p className='text-xl font-bold'>Institute: {instituteName}</p>
